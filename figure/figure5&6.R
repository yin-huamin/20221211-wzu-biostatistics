...
IDmapbitr<-bitr(anova.res_conserve$gene,fromType = "SYMBOL",
...
p.conserve <-k@result |> 
  dplyr::filter(ID %in% c("hsa04610","hsa00140","hsa00830","hsa04979","hsa00040")) %>% 
  arrange(p.adjust) %>% 
  ggplot()+
...
ggsave("conserve_kegg_top5.png",width = 5.5,height = 3,dpi=300)

g.conserve=try(enrichGO(IDmapbitr$ENTREZID,
               pvalueCutoff = 1,
               qvalueCutoff = 1,
               OrgDb = org.Hs.eg.db, 
               ont="all",
...
ggsave("conserve_go_top5.png",width = 14,height = 3,dpi=300)